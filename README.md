...
- `connect(to host: String, port: Int32, timeout: UInt = 0)` - This API allows you to connect to a server based on the `hostname` and `port` you provide. Note: an `exception` will be thrown by this function if the value of `port` is not in the range `1-65535`.  Optionally, you can set `timeout` to the number of milliseconds to wait for the connect.  However, this parameter will be ignored if the `Socket` is in blocking mode. Use the ``setBlocking` API defined below to turn off blocking.
